    ...
    return (
        <div className="w-full min-h-screen bg-neutral-50 text-neutral-900">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[260px_1fr] min-h-screen">
                ...
                <aside className="hidden md:flex flex-col gap-4 border-r bg-white p-4">
                    <div className="flex items-center gap-2">
                        <motion.div initial={{scale: 0.9, opacity: 0}} animate={{scale: 1, opacity: 1}}
                                    className="size-9 rounded-2xl bg-neutral-900"/>
                        <div>
                ...
                {showMobileNav && (
                    <aside className="fixed inset-0 z-20 bg-white p-4 flex flex-col gap-4 shadow-lg md:hidden">
                        <button
                ...
                <main className="flex flex-col">
                    ...
                    <header className="sticky top-0 z-10 flex items-center gap-3 border-b bg-white px-4 py-3">
                        ...
                        <div className="relative w-full max-w-xl">
                            ...
                            <input placeholder="Search leads, companies, messages..."
                                   className="pl-10 w-full rounded-2xl border px-3 py-2 outline-none focus:ring-2 focus:ring-neutral-300"/>
                        </div>
                    ...
                    {mockLeads.map((l, i) => (
                        <tr key={i} className="border-b hover:bg-neutral-50">
                            ...
                            <td className="px-4 py-3"><span
                                className="text-xs px-2 py-1 rounded-lg bg-neutral-100 border">{l.status}</span></td>
...
function AgentDraftAssistant() {
    return (
        <Card>
            ...
            <textarea placeholder="Message brief or key points..."
                      className="mt-3 min-h-28 rounded-xl border px-3 py-2"/>
            ...
            {reviewItems.map((r) => (
                <Card key={r.id}>
                    <div className="text-sm flex items-center gap-2">
                        ...
                        <span
                            className={`ml-auto text-xs ${r.risk === "High" ? "text-red-600" : r.risk === "Medium" ? "text-amber-600" : "text-green-600"}`}>{r.risk} Risk</span>
                    </div>
                ...
                {items.map((it) => (
                    <div key={it.name} className="rounded-2xl border p-4 bg-white flex items-center justify-between">
                        <div>
                            ...
                            <div
                                className={`text-xs ${it.status === "Connected" ? "text-green-600" : it.status === "Pending" ? "text-amber-600" : "text-neutral-500"}`}>{it.status}</div>
                        </div>
...
function GhostButton({children, icon: Icon}: React.PropsWithChildren<{ icon?: any }>) {
    return (
        <button
            className="justify-start gap-2 rounded-xl px-3 py-2 text-sm inline-flex items-center hover:bg-neutral-200">
...
function OutlineButton({children, icon: Icon}: React.PropsWithChildren<{ icon?: any }>) {
    return (
        <button
            className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm bg-white hover:bg-neutral-50">
            {Icon && <Icon className="size-4"/>}
            {children}
        </button>
    );
